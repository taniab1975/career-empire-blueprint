...
Provenance: Tania supplied both support replies on 2026-09-18. The substantive replies have no visible send date. Tripo's historical acknowledgement is dated 2026-09-15 15:24:32, and ticket creation displays Jackson at 15:24:31; these are not the date of the substantive response. Vendor statements below are supplied correspondence, not independently reproduced tests.

### Marvelous Designer / CONNECT: vendor-confirmed workflow
The response confirms that Marvelous Designer and EveryWear can prepare multiple interchangeable clothing items for a single base character driven by in-game skeletal animations. The base must contain joints and a mesh skinned to that skeleton. A-pose is highly recommended. Walk/Run clips are NOT required at initial garment creation.
1. Import the rigged base as FBX, retaining skeleton/joints and skin weights; prefer A-pose.
2. Create or dress and simulate each garment on the exact same base avatar so it drapes to the same body shape.
3. In EveryWear, optimize garment polygons as necessary and rig the garment to the base avatar's joints. Load compatible Walk/Run motions afterward, play or scrub in the Animation Editor, and inspect clipping and deformation. Correct affected skin weights with Weight Brush. The intended game behavior is that the shared skeleton animation drives the equipped clothing.
4. Pack UVs and bake the necessary texture maps.
5. Export the rigged garment as FBX through General Export, deselecting Avatar to omit the base body.
Vendor-supplied tutorial: https://www.youtube.com/watch?v=SkQcFpEV5Ao&t=1486s (reference retained; video not reviewed in this update).

This supersedes the current awaiting-Marvelous-Designer-response status; retain the September 14 pending entry as history. Trial access and an actual successful custom Tripo/EveryWear/Blender/game round trip are not established by the reply. It also clarifies the September 16 animated-FBX recommendation: importing existing motion remains an optional verification route, not a prerequisite for garment creation. Preserve the original animations in the Blender master and bring compatible motions in when testing rigging. Garment-only export does not by itself prove that no duplicate armature is exported or that bind transforms remain compatible.

### Tripo: prompt-adherence evidence and vendor advice
Tania reports that generation added breasts/swimwear, removed facial features and repeatedly produced T-pose despite the supplied A-pose reference/prompt; she reports turning AI off to avoid invention. The reference image and resulting model files are not attached to this update, so no new visual or model inspection is claimed. Her prompt sought the same proportions, face, neutral appearance, separated fingers/thumbs, palms-forward hands, fully formed feet, bald head and fitted short-sleeve mid-thigh neutral bodysuit, with deformation-friendly joints and no added anatomical exaggeration.
Tripo attributes these deviations to learned humanoid patterns, a tendency toward T-pose and weaker adherence to negative constraints. This is the vendor's explanation, not an independently verified cause or a verified meaning of the AI-off setting.
Vendor suggestions: use positive descriptions (bald, bare feet, neutral bodysuit, balanced androgynous proportions); put A-pose first and reinforce arms around 45 degrees, slightly bent elbows and relaxed hands; use a clear head-to-toe A-pose image or front/back/left/right views; try Smart Mesh, which the vendor says may better follow stylized geometry with cleaner lower-poly topology; generate the base then add bodysuit detail via Part Generation/texture tools; and use Free Retry, with 2–3 attempts suggested. The vendor says free retries are available on Tania's plan; current entitlement, credit cost and availability have not been independently checked. No retry, generation or spending is authorized or performed by this documentation update. Preserve the exact approved reference and master: staged generation can change them and is not a proven fix.
Tripo says this specific case was escalated to its model quality team for added anatomy/swimwear, deleted facial features and pose mismatch. This is separate from the September 14 standalone-clothing product suggestion; neither escalation promises a fix or release date. The historical seven-business-day acknowledgement is not a new response deadline starting today.

### Decision and next checkpoint
Vendor guidance now supports a rigged, skinned, same-base FBX → simulated garment → EveryWear weights/UVs/textures → garment-only FBX → original Blender skeleton → checked game GLB workflow. Acceptance still requires one matching garment to survive import/export with compatible scale, joints and bind transforms, then motion/clipping, equip/unequip and Studio/world validation. A-pose is preferred, not evidence that a new generation is required; inspect the existing master before changing its pose or rig. Prompt-only retries cannot guarantee exact reference preservation. Retain the selected Textured done lineage, complete unmasked fitting body, originals, prior experimental findings and rejected wardrobe evidence. Neither reply validates those rejected experiments or proves the full workflow on Career Empire assets.
Documentation-only update: both vendor responses recorded, active vendor-wait status corrected, September 16 workflow prerequisite clarified. No new visual assets, model tests, game edits or publication. Existing unrelated work and release checkpoints remain untouched.
